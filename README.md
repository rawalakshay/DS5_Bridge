...
| Two brief presses | Diagnostics for a third-party controller, typed into whatever text field has focus — point a text editor at it first. The first press types a report and walks you through a guided button capture, the second runs the rumble probe, and the third stops. Does nothing while a DualSense is connected; use the companion app for that. |
